(() => {
  ...
  async function gatherComponent(comp,target){const brand=R.top(state.board[target]);for(const src of comp){if(src===target)continue;const n=R.topRun(state.board[src]);for(let k=0;k<n;k++){const from=topCapCenter(src),to=landingCenter(target);state.board[src].pop();renderBoard();await flyCapBetween(brand,from,to,300,1);state.board[target].push(brand);renderBoard();maybeBeep(470,.025,.014);if(k<n-1)await sleep(105)}await sleep(80)}await sleep(100)}
  ...
  async function resolveAll(preferred){let anchor=preferred,guard=0;while(guard++<120){anchor=findResolvable(anchor);if(anchor<0)break;const comp=R.component(state.board,anchor);if(comp.length>1)await gatherComponent(comp,anchor);if(R.topRun(state.board[anchor])>=R.CLEAR_AT)await clearTopRun(anchor);else state.combo=0;anchor=null}}
